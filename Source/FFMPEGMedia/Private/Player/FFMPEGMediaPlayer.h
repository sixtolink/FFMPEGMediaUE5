...
private:
	...
	//用于间隔一定时间后调用
	TSharedPtr<FArchive, ESPMode::ThreadSafe> TempArchive;
	FString TempUrl;
	const IMediaOptions* TempOptions;
	const FMediaPlayerOptions* TempPlayerOptions;
	double TimeLength = 0;
	double IntervalTime = 5;//间隔时间
	bool bOpen = false;
	bool bNeedOpen = false;
	bool bIsArchiveOpen = false;
};
