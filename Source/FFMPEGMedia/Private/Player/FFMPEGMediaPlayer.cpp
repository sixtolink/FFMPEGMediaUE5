...
void FFFMPEGMediaPlayer::TickInput(FTimespan DeltaTime, FTimespan Timecode)
{
    if (bOpen)
    {
        TimeLength += DeltaTime.GetTotalSeconds();
        if (TimeLength > IntervalTime)
        {
            bOpen = false;
            if (bNeedOpen)
            {
                if (bIsArchiveOpen)
                {
                    Open(TempArchive.ToSharedRef(), TempUrl, nullptr);
                }
                else
                {
                    Open(TempUrl, TempOptions, TempPlayerOptions);
                }
            }
        }
    }
...
bool FFFMPEGMediaPlayer::InitializePlayer(const TSharedPtr<FArchive, ESPMode::ThreadSafe>& Archive, const FString& Url, bool Precache, const FMediaPlayerOptions* PlayerOptions )
{
    ...
    {
        {
            FScopeLock Lock(&ThisPtr->CriticalSection);
...
void FFFMPEGMediaPlayer::dumpFFMPEGInfo() {
   ...
   if ( FormatContext ) {
       ...
       for (unsigned int i = 0; i < FormatContext->nb_chapters; i++) {
           ...
           if (!ch) {
               UE_LOG(LogFFMPEGMedia, Warning, TEXT("Chapter %d is NULL!"), i);
               continue; 
           }
           sz_duration += FString::Printf(TEXT("    Chapter %d: "), i);
           if ( !ch->start) {
               UE_LOG(LogFFMPEGMedia, Warning, TEXT("Strat Time %d is NULL!"), i);
               continue; 
           }
